...
The elevated reach is a moving river, not a raised lake: `waterShadingGlsl.ts` carries the surface detail downstream along the profile map's channel tangent at a depth-scaled speed (a shallow bank drifts slower than the thalweg), and the rapids ribbons and edge lace evaluate in that same flow frame with the patchy grade gate, so riffle lines follow the bend instead of lying across it. `FacetedWater` and `NearWaterPatch` pass the tangent from `profileAt`; the coarse and near surfaces share one shading function and one set of new `createCoastalUniforms` values, so they cannot disagree at the patch rim. `HeadwaterFall.ts` draws the authored drop as a ballistic nappe pinned to the lip and landing, with stretched/converging threads, a crest highlight, a dissolving foot, impact plumes and deterministic textureless spray (`HeadwaterFallMist.ts`), and `waterShadingGlsl.ts` adds radial plunge rings on the horizontal pool. All of it is presentation over `NEVA_HEADWATERS`; `VisualRenderConfig.waterSurface.headwaters` owns the numbers, and no new render target or capture read is introduced.
