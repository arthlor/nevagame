...
The authored headwater fall is the one place where the water surface is steep rather than horizontal. `NEVA_HEADWATERS.fall` owns the lip, the 7.5 m falling segment and the landing; horizontal water discards that band and `HeadwaterFall.ts` draws the falling surface as its own sheet. The sheet pins its first and last rows to the live crest and the landing, and between them leaves the carved face for a ballistic arc — water departs the lip horizontally and accelerates under gravity — blended against the rock by `nappeDetach`, with a convex cross-section (`crossBulgeMeters`) and a spreading waist shaping the curtain. Falling threads stretch and converge as the water accelerates, a short crest highlight marks the roll-over, and the foot dissolves into vertical fingers over impact foam and rising plumes rather than ending on a cut line; spray puffs are deterministic and textureless, placed from the tier count. The shared water shading carries river surface detail downstream along the authored channel tangent at a depth-scaled speed, rapids ribbons and edge lace follow that same flow field, and plunge rings spread from the landing across the pool. The sheet shares the main water material's uniform objects so time, sun, weather, quality and reduced-motion changes cannot desynchronize the fall from the channel it connects to, never samples the opaque capture, and its normals come from its own grid rather than the ocean formula. `VisualRenderConfig.waterSurface.headwaters` owns every number: nappe detach, cross-section, width shape, per-tier rows and spray count, thread/breakup/crest/aeration/foot tuning, impact/apron/ring strength, river flow speed and depth response.
