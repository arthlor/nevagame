...
Representative state (`CURRENT_SCHEMA_VERSION = 53`, `world.layoutRevision = 25`):
```ts
interface GameState {
...
`GameState.mounts` includes the donkey and inherited horse carriage. `MountState.fishCargoSlotIds` exists only on the carriage; each non-null slot must agree with a `FishCargoState.location` of type `carriage`, its mount id and slot index. The validator checks both directions and refuses duplicate or orphaned links. Speed and animation phase remain transient; a reload resumes the saved pose and load at rest. `Carriage.ts` owns its finite capacity, footprint, movement tuning and interaction offsets.
