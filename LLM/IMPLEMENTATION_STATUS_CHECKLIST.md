...
- Save impact: **yes**. Migration required: **yes**. Schema v24 migrates layout 8 → 9, preserves valid X/Z and unrelated progression/economy/cargo/crop/boat/RNG truth, and relocates only land poses invalidated by the new water footprint. The current migration chain then applies v25. `tests/fixtures/save_v23_layout8.json` is the retained fixture.
...
### Character motion and interaction pass — 2026-08-31

- `char_player_a` keeps simulation/Rapier ownership unchanged while the presentation controller now preserves compatible locomotion phase, clears transient blends/contacts/springs after discontinuities without erasing the interaction one-shot that caused the pose jump, and resolves stance feet against the shared traversal-surface authority after the mixer and world transform.
- Fishing and rowboat hands use authored world-space grip constraints. Rowboat oars remain boat/oarlock-owned and animate around runtime oarlock pivots; mounted root placement derives from `rig_pelvis` rest height instead of a copied numeric offset.
- The selected player generator retunes walk/run/carry contact-pass-recovery poses, retains all catalog animation/socket contracts, restores gameplay-distance hat/hand readability, and enables volume-preserving player deformation. The published asset is mechanically valid, semantically deterministic, and `generated/reports/asset_budget_report.json` owns its current on-target budget result.
- Focused evidence passes **5 files / 108 tests** across animation/controller contracts, character pipeline, Rapier world/edge cases, and traversal/farm layout. Selected generation, atomic publication, and `art:determinism -- --asset char_player_a` pass. Repository-wide typecheck remains blocked by unrelated current UI/test prop drift; this pass has no gameplay/save mutation and requires no migration. Integrated animation/art judgment remains **Awaiting human game review**.
...
|---|---|---|---|
| P0 — Repository & Architecture | **Partial** | Deterministic simulation structure, content registry, input/mode separation, Three.js presentation, debug HUD, current schema/layout migration coverage through v25/layout 9 with historical topology fixtures, synchronized catalog/GLB publication provenance, terrain-focused tests, lint, and focused browser input checks. | Release-wide browser and technical-art certification remain open. |
...
| P2 — Persistence & Time | **Partial** | IndexedDB repository, current schema v25 / layout 9, migrate-then-validate, **primary + backup keys only**. Historical topology fixtures remain; the per-version ledger is owned by `01` §6.1. Weather-bounded offline progression and quest `nextQuestId` remain intact. | Durable simulation coverage exists; full browser save/reload release proof is still open. Overlay pause is not a serializable sim mode. |
...
| P14 — Final Art, Audio & UX Polish | **Reached / local UI overhaul implemented, unverified** | Catalog-driven art and audio remain intact. The coastal world-HUD/physical-interface overhaul is locally implemented across player-facing HUD, fishing/farming, title/recovery, dialogue, satchel, market, Hold & Stores, chart, journal/guide, expedition, pause/settings, confirmations, and landscape touch fallback; narrow simulation-owned HUD, market, crop/seed, forecast, sport-fight, satchel, logistics, chart, journal, pause, expedition, quest, and skill presentation queries plus atomic sell-all actions replace UI-owned derivation. | No tests, build, typecheck, browser automation, screenshots, or human visual review were run for this overhaul by explicit request. Requested viewport/UI-scale/long-text/keyboard/reduced-motion/touch review and audio/affected-scene approval remain open; no P14 completion claim. |
...
- [x] Schema version advanced to **23** in `01`, `03`, and this file. Code is `CURRENT_SCHEMA_VERSION = 23` (`src/persistence/SaveSchema.ts`); v22 retains the fish-quality/knowledge migration, while v23 rebases authored market commodities to the centered supply model and is covered by `tests/fixtures/save_v22_layout8.json` plus `tests/simulation/persistence.test.ts`.
