...
The authored headwater fall is the one place where the water surface is steep rather than horizontal. `NEVA_HEADWATERS.fall` owns the lip, the 7.5 m falling segment and the landing; horizontal water discards that band and `HeadwaterFall.ts` draws the same profile as its own sheet with arc-based streaks, breakup and landing foam, sharing the main water material's uniform objects so time, sun, weather, quality and reduced-motion changes cannot desynchronize the fall from the channel it connects to. The sheet never samples the opaque capture, so the refraction pass cannot feed back on itself, and its normals come from its own grid rather than the ocean formula. Nappe run, descent exponent, per-tier rows, streak and foam strength all live in `VisualRenderConfig.waterSurface.headwaters.fall`.
