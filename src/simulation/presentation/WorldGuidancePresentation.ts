...
export function buildNextWorldHint(state: GameState): WorldHintDto | null {
  ...
  const candidates: Array<WorldHintDto & { eligible: boolean }> = [
    { hintId: "hint.first_storm_at_sea", eligible: Boolean(boat && state.weather.type === "storm"), title: "Storm at Sea", message: "Rough water past her safe range starts a storm-helm gust: keep the skiff's bow into the wind with A/D until the gust passes, or ease the throttle and heave to. A survived gust recovers hull life; five failed gusts wreck the hull, which must be towed to Neva Harbor and repaired by Silas.", icon: "waves" },
