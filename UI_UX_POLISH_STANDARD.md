...
17. **Guildcraft Pointer Cursor (`src/ui/chrome/GuildcraftCursor.ts`, `guildcraftCursor.css`)**:
    - **Native Composited Pointer**: Fine-pointer desktops replace the system cursor with cursor-sized rasters derived at runtime from the published `guildcraft-pointer` sprite (29px arrow, tilted -45° up-left, tip at the hotspot). Cursor motion stays on the browser compositor, so no JS runs per `pointermove` and the pointer never drops frames with the game thread.
    - **Live States**: Hover, pressed and unavailable variants are generated from the same sprite and switched by `guild-cursor-hover` / `guild-cursor-pressed` / `guild-cursor-unavailable` classes. State comes from the existing DOM controls and the world canvas's own `cursor` affordance (including changes while the mouse is still), never from new gameplay state.
    - **Safe Fallbacks**: The native default cursor stays until the sprite loads and returns for coarse/touch pointers, forced-colors mode and sprite-load failure; every generated `cursor` value ends in the `auto` keyword. The cursor owns no input, simulation or saved state.
    - **Cascade Ownership**: `coastal.css` declares the `guild-cursor` layer before `neva-ui` (`@layer guild-cursor, neva-ui;`), so the `--guild-cursor*` image declarations outrank every HUD cursor rule regardless of selector specificity in both DEV and the production bundle.

18. **Work-Shift Timing Instrument (`LaborMinigameWidget.tsx`, `LaborShiftResult.tsx`, `coastal.css`)**:
    - **Compact Guildcraft Plaque**: The oscillating meter sits in a warm dark leather/slate instrument (`.labor-shift`) anchored bottom-center above the action prompt, with a gold serif station title beside the energy medallion and a beveled `+N Work` clean-strike reward chip. The prompt is suppressed while the instrument is visible (`.labor-shift` + `:has`), so one verb stays authoritative.
    - **Readable Sweet Spot**: The simulation-owned sweet-spot band renders as a striped amber window (`.labor-shift__sweet`) inside a recessed track, with a white needle on a 34ms linear transition so the 30Hz HUD cadence reads as continuous travel rather than stepped jumps.
    - **Hot State & Microinteractions**: While the needle is inside the band the plaque (`is-in-sweet`) brightens its frame, band and reward chip, turns the needle gold with a 260ms scale pulse, and swaps the hint to "In the sweet spot — strike now". Reduced motion drops the pulse and the needle transition.
    - **Strike Grade Feedback**: The `E` keycap on `[Strike]` matches the interact key; `[Stop]` stays a pointer/touch control because Escape is reserved for pause. After the strike ends the shift, `LaborShiftResult` rises and fades as one short `role="status"` plaque: `Clean strike +20 Work`, `Glancing blow +10 Work`, or `Missed` with the simulation reason.
    - **First-Shift Coaching**: Players are told about the minigame, not just shown it. `WorldGuidancePresentation` owns the one-time persisted `hint.labor_shift_timing` card that appears near an authored chore station ("strike when the needle crosses the gold band"), and `GameApp.announceLaborTimingOnce()` covers a player who starts a shift before that card fires. The Actions guide keeps a permanent written reference. Eligibility/availability decisions stay with the simulation; guidance only reads player state.
    - **Work Made Visible Beyond the Widget**: `RewardFeedbackPresentation` now samples the Work pool beside money/XP, so the world reward overlay floats `+N Work` / `−N Work` at the player, and the HUD Work bar pulses gold on gain and dims on spend (`.guild-work.is-gaining` / `.is-spending`, 680ms, removed under reduced motion).
