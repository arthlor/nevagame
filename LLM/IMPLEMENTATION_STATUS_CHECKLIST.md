...
### Full-regression triage in progress

- The direct Vitest run in `output/audit-full-regression-NS4UdT/run.log` is still running; no complete result or passing full-suite claim exists. Its already-completed HUD files exposed five stale assertions in `empirical_m2_hud` and one in `hudNotifications`. Those two files now exercise the current almanac, storm identity/severity, menu shortcut, Work meter values, exact selected tool and readout. The clock cases render the live DTO-to-HUD path with independent boundary expectations instead of a retired decoration and copied formula. The unavailable-seed assertion targets slot 2 specifically, so an unrelated empty fertilizer slot cannot satisfy it. A separate direct run passes **41 tests in two files**; TypeScript and focused diff checks pass, and ESLint reports zero errors with one pre-existing `any` fixture warning. Evidence: `output/audit-repair/hud-empirical-final.log`, `hud-empirical-types.log` and `hud-empirical-lint.log`. Only these already-completed test files and documents changed while the baseline continued; runtime source was kept unchanged. This is semantic regression coverage, not fresh browser interaction or visual proof.
- Completed persistence-related files additionally report migration timeouts in `persistence`, `harborCoastMigration`, `farmingPerfection`, `questPersistence`, `huntFixes2026`, `gameplayFixes` and `workProgressionPass`. These are not classified as functional save corruption or harmless harness limits. The first independent harbor recovery case takes about 104 seconds in this contended run while its warm cases take milliseconds. Preserve the 30-second limit until the expensive work is isolated.
- A cold Node inspector profile now confirms the shared cost: `migrateTerrainLayout13` calls `createWorldEnvironmentLayout(seed).staticPlacements` for collision inputs, but eagerly generates Neva and Sunreach ground cover. The independent retained v32 fixture takes 34.73 seconds in migration, with 25.17 seconds of inclusive sampled time in the two ground-cover generators. The script verifies input immutability, migrated-envelope validity and idempotence, and records full migrated-state, static-placement, cover and collision hashes for a subsequent parity comparison. Evidence: `output/audit-repair/profile-cold-migration.ts`, `cold-migration-before.json` and `cold-migration-before.cpuprofile`. This contended Node run is not browser load or production frame-time evidence. Preserve the full seeded collision inputs and compare hashes before admitting a static-only composition path; no migration, timeout or gameplay behavior is changed by this triage record.
...
| P10 — Progression & Contracts | **Partial** | Five persisted rods, ordered harbor purchase/equip, proficiency XP/ranks, quality-scaled harvest XP, hard discounted Work costs, seven contract templates with owned delivery markets, journal, gated expedition board, explicit 18-quest `nextQuestId` chain, data-owned costs/rewards, validated objectives, and live skiff purchase. | Broader balance and long-session progression proof remain open. |
