...
4. `LLM/LLM_AGENT_ART_PIPELINE_INSTRUCTIONS.md` — 3D/procedural/rendering production authority: generator-to-GLB workflow (authored Three.js generators first, legacy Blender generators until ported), generator/spec rules, `VisualRenderConfig`, optimization and visual regression/style-match implementation.
...
8. `LLM/BLENDER.md` — operational authority for catalog-driven production (both producers), validation, publishing, reports, Art Yard handoff, and runtime integration. `tools/authored/README.md` owns how authored Three.js generators are written and ported.
...
| Existing asset or new asset in an existing family | Full `BLENDER.md` and `tools/authored/README.md` (plus `tools/blender/README.md` while the family is still a legacy Blender generator), selected catalog entry, generator and runtime integration point. Read the isolated sheet and relevant `04` section when appearance changes. |
...
Use the repository copies under `.agents/skills/threejs-*`; read `.agents/skills/CONVENTIONS.md` with the selected skill. The 24 graphics skills provide techniques and examples. Neva's authorities still own scope, art direction, gameplay truth, asset publication, renderer configuration and verification. The upstream pack does not provide gameplay, UI, provider-generation or release agents.
...
For a broad graphics pass, use `.agents/skills/threejs-skill-router/SKILL.md` to select only the relevant specialists. After the catalog, isolated sheet, and owning generator, Codex may use `.agents/skills/threejs-procedural-geometry/SKILL.md` plus its `references/geometry-craft-workflow.md` and `references/geometry-quality-gates.md` as technique guidance. Implement in the authored generator and `tools/authored/kit/` (porting a legacy Blender family first when that is its current home); do not import the skill's portable geometry kit or add a parallel export path. Provider APIs need an explicit human request; never publish a downloaded GLB. Use `03` for release and visual-gold gates.
