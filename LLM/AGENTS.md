...
2. [`LLM/02_GAMEPLAY_SYSTEMS_IMPLEMENTATION.md`](02_GAMEPLAY_SYSTEMS_IMPLEMENTATION.md) — gameplay/balance/math and narrative-mechanics authority: farming, crop lifecycle, fishing, cargo/boats, markets, Work Capacity, progression, contracts, the authored quest/lore spine, dialogue contract, and vertical-slice rules.
...
- `src/render/config/VisualRenderConfig.ts` owns the live renderer baseline, including terrain/road supporting-map sampling and blend strengths.
...
Current implementation snapshot: the live save schema is 19, the authored world
...
live render configuration as owners, not this snapshot.

Routine existing-asset work reads `AGENTS.md`, `LLM/BLENDER.md`, `tools/blender/README.md`, the selected catalog entry, owning generator, runtime integration point, and only the directly relevant Art Bible section. Story-relevant design, world-layout, and runtime-integration work additionally reads the narrative contract in `02` and environmental-storytelling section in `04`; a routine generation-only prompt still follows the lean generate-asset route and leaves `02`/ArcheAge unread. In-game layout / Place / F2 editor work reads `LLM/LAYOUT_EDITOR.md` plus `src/layout-editor/`, `src/app/PlacementEditor.ts`, and `tools/layout-editor/patchPlacement.ts`. New/shared generator, renderer/material, gameplay-contract, and release/gold-slice work adds the owning full canonical documents. Ground supporting-map work is renderer/material work and must also read Art Pipeline section 6.2 plus `ExternalSurfaceTextures.ts`. Image/study-guided assets keep a required selected-entry `referenceAuthoring` brief; do not read or emit unrelated asset briefs.
...
6. **Art & Shader Compliance** — production visuals must conform to `04` + Art Pipeline, the canonical `PaletteTokens`/`PaletteMaterials` vocabulary and the shared `VisualRenderConfig`. Avoid arbitrary production colors/materials, per-scene exposure/tone-map hacks, photoreal texture drift, and normal-world toon/ink outlines. Ground supporting maps are allowed only as the Art Bible's remapped low-frequency tiler; photographic RGB is not a legal final diffuse.
7. **Visual Identity Is Early Infrastructure** — do not postpone the real visual system until P14. P0.5 establishes rendering/materials; P0.75 accepts the bridge/farm/harbor/coast visual-gold slices before broad asset/world production, while technical-art certification remains a separate release gate.
...
| --- | --- | --- |
| `@LLM @tools generate assets of …` or any catalog/Blender asset generation | Do **not** start with `threejs-game-director`. After Neva catalog + isolated sheet + owning generator, optionally load `.agents/skills/threejs-aaa-graphics-builder/references/checklists/procedural-model-quality.md` (and `model-recipes.md` when appearance is being designed) as critique vocabulary | Prefer the repo copy `.agents/skills/` over `~/.codex/skills`. Translate critique into the registered Blender family generator and `authored.py`. Polyfork is not an exporter. Provider skills never publish a direct GLB. `threejs-qa-release` is release/gold-slice only. |
...
1. Read the root `AGENTS.md` and the task-owning Neva source-of-truth files before applying an external skill. Prefer the repo copy `.agents/skills/<name>/SKILL.md` over `~/.codex/skills` when they differ. The Neva authority order above always wins if the external pack disagrees.
