...
| Existing asset or new asset in an existing family | Full `ASSET_PRODUCTION.md` and `tools/authored/README.md`, selected catalog entry, generator (or committed authored GLB source) and runtime integration point. Read the isolated sheet and relevant `04` section when appearance changes. |
...
Isolated studio sheets under `tools/art/references/isolated/` are style-match evidence for the mapped catalog ID. Numbered crop/diorama PNGs in the references README are graphics-only extracts from `art-reference.png`; catalog IDs win if that README drifts.
