...
- Tomato/potato stages remain conditional crop presentation, and `boat_skiff_a` remains registered but is not created by a fresh save. These are intentionally distinct from static world placement.
...
- Fresh saves retain the physical docked rowboat as world content, but boarding is locked until the commission reward is granted. Schema **10** migrates the feature-unlock list, the one-time Act 5 starter-school flag, and the harbor fish-table structure.
...
- Passing focused evidence: `tests/simulation/questDomain.test.ts`, `tests/simulation/questPersistence.test.ts`, `tests/simulation/gameplayFixes.test.ts`, `tests/unit/worldLayout.test.ts`, and `tests/unit/assetPlacementAudit.test.ts` — current Vitest suite is **~262** `it`/`test` cases (grep 2026-08-25); do not treat the old 75-test focused run as the full gate.
- Additional targeted evidence: `tests/simulation/simulationLoop.test.ts` (1 test) and `tests/simulation/persistence.test.ts` (12 tests) pass.
- `npm run art:codegen:check` and `npm run typecheck` pass; the generated catalog projection reports 92 assets.
- `npm run build` and `npm run lint` pass; build retains the existing large Rapier chunk warning.
- Browser runtime quest-flow proof, integrated gameplay-camera review, complete cross-browser gates, and human visual approval remain outstanding. Static/type/test evidence does not replace those gates.
...
| Chromium gameplay/control E2E | **Last Playwright browser-loop failed (P12 not proven)** | Do not mark P12 complete. Do not cascade this into empty/Red for P0–P11 systems that exist in src. |
...
|---|---|---|---|
| P0 — Repository & Architecture | **Partial** | Deterministic simulation structure, content registry, input/mode separation, Three.js presentation, debug HUD, typecheck, lint, and build are present. Schema 11 / layout 3. | Typecheck expected green. Full P0 milestone still wants a current passing suite + Playwright; last Playwright loop failed. Do not treat the old 212-case fail snapshot as current. |
...
| P6 — Boats | **Partial** | Rowboat/skiff definitions, boarding/driving/docking, persistence, and Act 4 commission exist. Physics may sample WaterSurface for boat bob; canonical boat.y stays waterline. | Skiff is unpurchasable. Emergency Tow / hook-class not live. Browser dock/board/sail loop not proven. |
...
| P10 — Progression & Contracts | **Partial** | Five rods, proficiency XP/ranks, five contract templates, journal, gated expedition board, explicit quest `nextQuestId` chain and feature unlocks. Rank unlock tables (`farmingUnlocks` etc.) are unused; LIVE gates are `crop.minimumFarmingXp` and a few `recipe.minimumSkill`. No buy-rod/buy-boat. | Three capability-changing milestones not proven in a new-save browser run. |
...
| P12 — Full Vertical-Slice QA | **Red / not proven** | Systems through P11 exist in src; content counts look like P13 tables. | Last Playwright browser-loop failed. Do **not** mark P12 complete. No passing new-save wheat-through-reload browser proof. |
...
| P14 — Final Art, Audio & UX Polish | **Not reached / partial polish exists** | 92 catalog assets, character/crop presentation, fish-school presentation, VFX, UI polish, accessibility/reduced-motion work, ambient/farming audio, interiors, vegetation, boats, and world dressing exist. | P12 is not green; 19 assets are below target; asset coverage is incomplete; audio covers ambience/farming/processing but not the complete fishing/boat/market loop; no current human in-game approval. |
| P15 — Performance & Browsers | **Partial** | Build size is under the initial-load target; Meshopt, batching, instancing, and performance diagnostics exist. Stored four-scene benchmark is under triangle/draw targets but uses SwiftShader. | Last debug capture reported 150 draws and 1,229,530 triangles (under 1.5M hard max, above 900k target). Browser matrix incomplete; last Playwright failed. SwiftShader FPS is not hardware proof. |
